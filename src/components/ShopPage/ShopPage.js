...
// Here ADD CardPerPage NUMBER****** For example I have added [4] *********************
  ...
  const displayCards = allPost.slice(pagesVisited, pagesVisited + cardPerPage).map(propsData => <ShopPageCard key={propsData._id} propsData={propsData} />)

  const pageCount = Math.ceil(allPost.length / cardPerPage)
    ...
    return (
        <div>
            ...
            <div className="shop__page__container container">
                ...
                <div className="row">
                    {/* {allPost.length ?
                    allPost.map(propsData => <ShopPageCard key={propsData._id} propsData={propsData} />)
                    : <TableSpinner />
                    } */}
                    {displayCards} 
